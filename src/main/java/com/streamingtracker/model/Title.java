package com.streamingtracker.model;

import java.util.List;

public class Title {
    public int id;
    public String imdbId;
    public String posterUrl;
    public String backdropUrl;
    public String name;
    public Integer year;
    public Integer runtime;
    public String director;
    public List<String> actors;
    public List<String> genres;
    public String originalLanguage;
    public Double imdbRating;
    public Integer imdbVotes;
    public String ageRating;
    public String shortDescription;
    public String releaseDate;    // ISO "YYYY-MM-DD", earliest known release, may be null
    public String streamingDate;  // ISO "YYYY-MM-DD", most recent availableFromTime across providers, may be null
    public String firstSeen;      // ISO "YYYY-MM-DD", date title first appeared in a scraper run
    public String contentType;          // "movie" or "show"
    public Integer numberOfSeasons;    // shows only; null for movies. Counts only seasons with at least one aired episode (a real originalReleaseDate) — JustWatch lists renewed-but-unreleased seasons as stub entries (often a single placeholder episode with no date), which would otherwise inflate this.
    public Boolean nextSeasonAnnounced; // shows only; true when JustWatch has a season beyond numberOfSeasons that hasn't aired yet (renewed, not released) — powers the "Coming Soon" section
    public List<String> providers;
    public List<String> audioLanguages; // language codes available across streaming offers

    // shows only; excluded from /titles Gson serialization (transient), written separately to /episodes
    public transient List<Episode> episodes;
}
